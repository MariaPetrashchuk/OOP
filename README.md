# 🐍 Репозиторій з дисципліни «Об’єктно-орієнтоване програмування (ООП)»

## 📌 Призначення репозиторію

Цей репозиторій створено для зберігання навчальних робіт з дисципліни **«Об’єктно-орієнтоване програмування»**.

Тут містяться:
- лабораторні роботи;
- практичні завдання;
- навчальні проєкти;
- приклади програм на Python.

Під час навчання використовуються:
- 🌐 GitHub — для зберігання коду;
- 💻 Visual Studio Code — для розробки;
- 📝 Markdown — для документації;
- ⚙️ Python та додаткові інструменти для програмування.

---

## 👤 Власник репозиторію

- **Студент:** Петращук Марія
- **Навчальний заклад:** ВСП Фаховий коледж інформаційних технологій НУ «Львівська політехніка»
- **Група:** КН-33

---

## 🧪 Лабораторні роботи

- 📁 [Lab 2](./lab_2/README.md)
- 📁 [Lab 3](./lab_3/README.md)
- 📁 [Lab 4](./lab_4/README.md)
- 📁 [Lab 5](./lab_5/README.md)
- 📁 [Lab 6](./lab_6/README.md)
- 📁 [Lab 7](./lab_7/README.md)
- 📁 [Lab 8](./lab_8/README.md)

---

## 📂 Додаткові проєкти

- 📁 [06 Python Agents](./06_python_agents/)
- 📁 [Agent](./Agent/)
- 📁 [Code Pipeline](./code_pipeline/)
- 📁 [MyPy Project](./mypy_project/)
- 📁 [Research Team](./research_team/)
- 📁 [Smart Pipeline](./smart_pipeline/)
- 📁 [Stateful Agent](./stateful_agent/)
- 📁 [Story Improver](./story_improver/)
- 📁 [Temperature Agents](./temperature_agents/)
- 📁 [Zalik](./zalik/)


---

## 📚 Основні теми курсу

- Основи Python
- Об’єктно-орієнтоване програмування
- Класи та об’єкти
- Наслідування
- Поліморфізм
- Інкапсуляція
- Git та GitHub
- Markdown
- MyPy та Flake8

---

## 📖 Корисні посилання

- GitHub Docs — https://docs.github.com/
- Markdown Guide — https://www.markdownguide.org/
- VS Code Docs — https://code.visualstudio.com/docs
- Python Docs — https://docs.python.org/3/

---

© 2026 Петращук Марія | Репозиторій для навчальних робіт з ООП
